...
def preprocessing(p1s, p2s, i, img):
    ...
    # cv.destroyAllWindows()
    # # 闭运算
    # kenerlX = cv.getStructuringElement(cv.MORPH_RECT, (5, 1))
    # image = cv.morphologyEx(dst, cv.MORPH_CLOSE, kenerlX, iterations=1)
    ...
    # image = cv.erode(image, kernelX, iterations=1)
    # image = cv.dilate(image, kernelX, iterations=1)
    #
    ...
    # cv.imshow('contour_image', origin_copy)
    # cv.waitKey()
    # cv.destroyAllWindows()


    # rotate_image = rotate_plate_image(contours[0],warped.copy())
    ...
    if box[3][0] < left_min2:
        if box[3][0] < left_min1:
            ...
            left_index2 = 3
            left_min2 = box[3][0]
    # print("left_min1:", left_min1)
    # print("left_min2:", left_min2)
    # 左边
    ...
    # print("左上点的坐标为：(" + str(left_up[0]) + "," + str(left_up[1]) + ")，右下点的坐标为(" + str(right_down[0]) + "," + str(
    #     right_down[1]) + ")")
    # print("左下点的坐标为：(" + str(left_down[0]) + "," + str(left_down[1]) + ")，右上点的坐标为(" + str(right_up[0]) + "," + str(
    #     right_up[1]) + ")")
    # 处理后的图片的宽和高
    width, height = 200, 60
    # pts1 是倾斜的图片的四个点的坐标
    ...
    # cv.imwrite("./data/images/after", warped)
    # cv.imshow('warped', warped)
    # cv.waitKey()
    # cv.destroyAllWindows()
